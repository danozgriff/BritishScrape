...
#D-1
       ...
       d1list = scraperwiki.sqlite.execute("select `TIDM`, `Price`, `Date`, substr(Date, 1, 4) from Signal_History where tidm = '" + tidm + "' and substr(Date, 1, 2) = '%s' and substr(Date, 4, 2) = '%s'" % (time.strftime('%~d',d1date), time.strftime('%~m',d1date)))
...
if 1==0:
     ...
     d1date=ndate - datetime.timedelta(days=10)
     print d1date
     print "select `TIDM`, `Price`, `Date` from Signal_History where tidm = '" + tidm + "' and strftime('%Y', date)= ?", str(d1date.year)
     #ndate=datetime.date.today()
     #print ndate.year
     #strftime('%Y', ndate)
#------------------------------------------------
