...
#D-1
       ...
       print " "
       d1list = scraperwiki.sqlite.execute("select `TIDM`, Max(Date) from Signal_History where tidm = '" + tidm + "' and Date < '%s/%s/%s' group by `TIDM`" % (d1date.year, d1date.month, d1date.day))
