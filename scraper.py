...
#D-1
       ...
       d1list = scraperwiki.sqlite.execute("select `TIDM`, `Price`, `Date`, substr(Date, 1, 4) from Signal_History where tidm = '" + tidm + "' and substr(Date, 1, 2) = '%s' and substr(Date, 4, 2) = '%s'" % (strftime('%~d',d1date), strftime('%~m',d1date)))
